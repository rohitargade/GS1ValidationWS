...
import com.ascention.validationWS.dao.ProductCoreDAO;
@Repository
public class ProductCoreRepository implements ProductCoreDAO{
    @Autowired 
    private NamedParameterJdbcTemplate jdbcTemplate;
	public ProductCore getAllProductAttributes(Integer Product_Id, Integer Product_Hierarchy_Id,
        ...
        String sql = "SELECT		PC.[Product_Id]\r\n" + 
        		...
        		"AND			PC.[Is_Active] = 1\r\n" + 
        		"AND			PH.[Product_Hierarchy_Id]=:Product_Hierarchy_Id;" 
        		;
        System.out.println(sql);
        
        return (ProductCore)jdbcTemplate.queryForObject( sql, parameters, new BeanPropertyRowMapper(ProductCore.class));  
...
class ProductCoreMapper extends BeanPropertyRowMapper<ProductCore> {

	@Override
	public ProductCore mapRow(ResultSet rs, int rowNum) throws SQLException {
		ProductCore product = new ProductCore();
		product.setProduct_Id(rs.getInt("Product_Id"));
		product.setProduct_Hierarchy_Id(rs.getInt("Product_Hierarchy_Id"));
		return product;
	}
